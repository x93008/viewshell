target_link_libraries(viewshell PUBLIC nlohmann_json::nlohmann_json tl::expected)

if(WIN32)
  find_package(webview2-sdk CONFIG REQUIRED)
  target_link_libraries(viewshell PRIVATE webview2-sdk::webview2-sdk)
  target_compile_definitions(viewshell PRIVATE VIEWSHELL_HAS_WEBVIEW2=1)

  target_link_libraries(viewshell PRIVATE ole32 user32 shell32 dwmapi)
elseif(APPLE)
  find_library(APPKIT_FRAMEWORK AppKit REQUIRED)
  find_library(FOUNDATION_FRAMEWORK Foundation REQUIRED)
  find_library(WEBKIT_FRAMEWORK WebKit REQUIRED)
  target_link_libraries(viewshell PRIVATE ${APPKIT_FRAMEWORK} ${FOUNDATION_FRAMEWORK} ${WEBKIT_FRAMEWORK})
else()
  pkg_check_modules(GTK3 REQUIRED gtk+-3.0)
  pkg_check_modules(X11 REQUIRED x11)
  pkg_check_modules(XI REQUIRED xi)
  target_include_directories(viewshell PRIVATE ${GTK3_INCLUDE_DIRS} ${X11_INCLUDE_DIRS} ${XI_INCLUDE_DIRS})
  target_link_libraries(viewshell PRIVATE ${GTK3_LIBRARIES} ${X11_LIBRARIES} ${XI_LIBRARIES})
  target_compile_options(viewshell PRIVATE ${GTK3_CFLAGS_OTHER} ${X11_CFLAGS_OTHER} ${XI_CFLAGS_OTHER})

  pkg_check_modules(WEBKITGTK41 QUIET webkit2gtk-4.1)
  if(WEBKITGTK41_FOUND)
    target_include_directories(viewshell PRIVATE ${WEBKITGTK41_INCLUDE_DIRS})
    target_link_libraries(viewshell PRIVATE ${WEBKITGTK41_LIBRARIES})
    target_compile_options(viewshell PRIVATE ${WEBKITGTK41_CFLAGS_OTHER})
  else()
    pkg_check_modules(WEBKITGTK40 REQUIRED webkit2gtk-4.0)
    target_include_directories(viewshell PRIVATE ${WEBKITGTK40_INCLUDE_DIRS})
    target_link_libraries(viewshell PRIVATE ${WEBKITGTK40_LIBRARIES})
    target_compile_options(viewshell PRIVATE ${WEBKITGTK40_CFLAGS_OTHER})
  endif()
endif()
